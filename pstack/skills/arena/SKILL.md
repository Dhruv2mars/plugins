...
3. Pick the runner count. All runners inherit the session model (GLM-5.3-Flash); omit `model` on every candidate. Diversity comes from isolated contexts and separate worktrees, not model slugs. Default N=3-4. Spawn more when the arena covers multiple design directions.
...
After all Phase B candidates complete, spawn one readonly judge subagent on the session model (omit `model`). It sees the rubric and the candidates by path label, scores each criterion, and recommends a base with rationale. It runs in parallel with the parent's reading in Phase D, not with the candidates themselves. Spawning while candidates are still writing means the judge sees partial or empty outputs and reports them as dropouts.
