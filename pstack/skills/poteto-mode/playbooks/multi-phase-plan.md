...
3. Explore in subagents with `subagent_type: "poteto-agent"` and no model key so each inherits the session model (the **guard-the-context-window** principle skill). Each returns file pointers, conventions, test commands, and entry points. No inlined dumps.
...
- [ ] Arm the 30-minute audit tick. Arm it as a ZCode scheduled automation (CronCreate) or a monitored-shell sleep loop. Never leave the cadence to memory.
...
- [ ] Strip slop from the diff (unslop-style slop-strip) before each commit and run `/no-comments` before review.
...
Each live lane runs in its own git worktree at the PR head. Drive through the matching control skill (browser-use `control-browser` for UI surfaces, a repo-local skill for CLIs).
...
**Verify, live.** Tests alone are not sufficient verification. A PR is verified only when its unit, live, and perf boxes are all checked. Ten lanes on the session model at the PR head, per the boot recipe.
