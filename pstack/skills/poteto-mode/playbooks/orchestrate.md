...
- **Worker / verifier.** Always a local background subagent in its own git worktree, unless the work is readonly (coverage scans, reviews may share the checkout). Local-only needs: runtime verification via the control skills, reading local transcripts under `agent-transcripts/`, Simulators and local IDE state. Auth that exists only here. Remote-state briefs inline what they need or point at repo paths. Prefer fewer, broader workers. One writer per worktree or branch (principle-separate-before-serializing-shared-state). Run a unit's verifier as a separate readonly subagent, never the writer checking itself.
...
- Never resume an agent to check on it. A resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, the background agents status in the task list. Transcript mtime is not liveness.
