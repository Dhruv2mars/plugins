...
**You own pixel-exact equivalence. The baseline is the spec; you do not touch it.** Design intent comes from the **frontend-design** skill; the baseline remains the spec. Equivalence is verified by image diff, not by eye.
...
4. Verify each component against its baseline via image diff on the matching surface via the control skill. A nonzero diff is a fail. Investigate the pixel delta. Iterate per component until the diff is zero.
