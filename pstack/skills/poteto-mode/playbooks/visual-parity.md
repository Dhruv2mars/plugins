...
**You own pixel-exact equivalence. The baseline is the spec; you do not touch it.** For "make X match Y exactly", styling-system migrations, porting a UI across frameworks. Design intent comes from the **frontend-design** skill; the baseline remains the spec. Equivalence is verified by image diff, not by eye.
...
4. Verify each component against its baseline via image diff on the matching surface via the control skill. A nonzero diff is a fail; investigate the pixel delta, don't wave it through. Iterate per component until the diff is zero.
