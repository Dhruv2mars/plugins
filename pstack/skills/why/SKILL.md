...
Before spawning investigators, list the available MCPs from the ZCode environment. Use the available-tools map when present. Otherwise inspect the harness tool listing for enabled MCP servers.
...
- `model`: omit — inherits the session model (GLM-5.3-Flash)
